...
'''
def imagesearch_count(image, precision=0.75):
    angles = [0,90,180,270]
    directions = ['up', 'left', 'down', 'right']
    returnObj = []
    for i in range(4):
    	if (image == 'block.png' and i > 0):
    		break
	    img_rgb = pyautogui.screenshot()
	    img_rgb = np.array(img_rgb)
	    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
	    template = cv2.imread(image, 0)
	    template = rotate(template,angles[i])
	    ...
	    returnObj.append(dictionary)
	    for pt in zip(*loc[::-1]):  # Swap columns and rows
	        cv2.rectangle(img_rgb, pt, (pt[0] + w, pt[1] + h), (0, 0, 255), 2) #// Uncomment to draw boxes around found occurances
	    cv2.imwrite('result'+directions[i]+'.png', img_rgb) #// Uncomment to write output image with boxes drawn around occurances
    return returnObj
